...
import { netMonthly, netYearly, yieldPctYearly, fmtMoney } from '../../core/domain/calc';
...
export default function ObjectCreateSheet({ onClose }: Props) {
    ...
    const draft = useMemo<Objectvestment>(() => ({
        ...
        name, kind: 'OBJECT',
        purchasePrice, grossGainMonthly, costMonthly,
        netGainMonthly: '0', netGainYearly: '0', yieldPctYearly: '0'
    ...
    return (
        <div className="sheet" onClick={onClose}>
            <div className="sheet__panel" onClick={e=>e.stopPropagation()}>
                <div className="h1" style={{marginBottom:8}}>Investment hinzufügen</div>
                ...
                <div className="grid">
                    ...
                    <div className="row">
                        <div style={{flex:1}}>
                            ...
                            <input className="input" value={purchasePrice} onChange={e=>setPurchasePrice(e.target.value)} />
                        </div>
                        <div style={{flex:1}}>
                            <div className="label">Gewinn/Monat (€)</div>
                            <input className="input" value={grossGainMonthly} onChange={e=>setGrossGainMonthly(e.target.value)} />
                        </div>
                    </div>

                    <div>
                        <div className="label">Kosten/Monat (€)</div>
                        <input className="input" value={costMonthly} onChange={e=>setCostMonthly(e.target.value)} />
                    </div>

                    <div className="kpi">
                        <div><b>Netto monatlich</b><br/>{fmtMoney(netM)} €</div>
                        <div><b>Netto jährlich</b><br/>{fmtMoney(netY)} €</div>
                    </div>
                    <div style={{marginTop:6}}><b>Rendite p.a.</b> {yld} %</div>
